...
const createPath = function(isProd, suffix) {
  ...
  if (isProd) result = `${result}/production`;
  else result = `${result}/development`;
...
const filesToIgnore = ['angular-chart.js', 'css3-animate-it.js', 'Chart.js'];
...
const jsHelperFunc = function(file, enc, next) {
  ...
  if (
    _.find(filesToIgnore, function(currIgnoreName) {
      return currIgnoreName === filename;
    })
  ) {
    next(null, file);
  } else {
